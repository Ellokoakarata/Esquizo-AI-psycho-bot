...
def register_message_handlers(bot: TeleBot):
    ...
    def handle_message(message):
        try:
            ...
            user_id = str(message.from_user.id)  # Usar message.from_user.id para obtener el ID del usuario
            chat_id = str(message.chat.id)       # ID del chat (puede ser un grupo o chat individual)
            ...
            logger.info(f"Mensaje recibido de {user_name} (User ID: {user_id}, Chat ID: {chat_id}): {message.text}")
            ...
            if reply_content:
                ...
                send_message_with_retries(bot, chat_id, "No tengo una respuesta clara. Hoy el caos es extraño.")

            logger.info("Procesamiento del mensaje completado")

        except Exception as e:
            logger.error(f"Error al procesar el mensaje: {e}", exc_info=True)
            send_message_with_retries(bot, chat_id, "Lo siento, estoy experimentando dificultades técnicas. Por favor, intenta de nuevo más tarde.")
